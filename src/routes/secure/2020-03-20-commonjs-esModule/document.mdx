...
模块查找过程会将软链替换为系统中的真实路径，例如 `lib/foo/node_moduels/bar` 软链到 `lib/bar`，`bar` 包中又 `require('xxx')`，最终运行 `foo` module 时，`require('xxx')` 的查找路径是 `lib/bar/node_moduels/xxx` 而不是 `lib/foo/node_moduels/xxx`。
...
![image](http://git.caimi-inc.com/trinity/discussions/uploads/83ae3e0fce046b89b33825090a4e39a1/image.png)
...
* `CommonJS` 可以在运行时使用变量进行 require, 例如 `require(path.join('xxxx', 'xxx.js'))`，而静态 `import` 语法不行，因为 ES6 模块会先解析所有模块再执行代码。

![image](http://git.caimi-inc.com/trinity/discussions/uploads/0bc75e72b651ce12804a4cf2876802f3/image.png)
