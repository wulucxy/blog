...
export default async function Home() {
  ...
  return (
    <div className="mt-14 relative -top-[10px] flex flex-col gap-8">
      {posts.map((post) => (
...
function PostTitle({ post }) {
  ...
  return (
    ...
    >
      <Link className="block no-underline" href={"/" + post.slug + "/"}>
        {post.title}
      </Link>
    </h2>
  );
...
function PostMeta({ post }) {
  return (
    <p className="text-[13px] text-gray-700 dark:text-gray-300 mb-0">
